...
const calculateFiscalResponsibilityScore = require('./utilities/calculateFiscalResponsibilityScore');
...
app.get('/api/fiscal-responsibility-score', async (req, res, next) => {
  try {
    let cursor = null;
    let added = [];
    let modified = [];
    ...
    for (const transaction of allTransactions) {
      ...
      accountTransactions[accountId].push(transaction);
    }
    ...
    for (const account of accounts) {
      ...
      const score = calculateFiscalResponsibilityScore(transactions);
      accountScores.push({
        accountName: account.name,
        fiscalResponsibilityScore: score,
      });
    }

    res.json(accountScores);
  } catch (error) {
    next(error);
  }
});
